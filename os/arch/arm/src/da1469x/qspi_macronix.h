...
__STATIC_INLINE uint8_t flash_mx_read_security_register(HW_QSPIC_ID id) __attribute__((always_inline)) __UNUSED;
__STATIC_INLINE uint8_t flash_mx_read_security_register(HW_QSPIC_ID id)
{
...
__STATIC_INLINE void flash_mx_enable_quad_mode(HW_QSPIC_ID id) __attribute__((always_inline));
__STATIC_INLINE void flash_mx_enable_quad_mode(HW_QSPIC_ID id)
{
        uint8_t status;
        ...
        if (!(status & MX_STATUS_QE_MASK)) {
                ...
                flash_write_status_register(id, status | MX_STATUS_QE_MASK);
        }
}

__STATIC_INLINE void flash_mx_set_output_driver_strength(HW_QSPIC_ID id, uint8_t ods_value) __attribute__((always_inline)) __UNUSED;
__STATIC_INLINE void flash_mx_set_output_driver_strength(HW_QSPIC_ID id, uint8_t ods_value)
{
        ...
        status_reg = flash_read_status_register(id);  // This will be used when setting the dummy bytes.
        flash_transact(id, cmd, 1, &conf_reg, 1);        // This will be used when setting the dummy bytes.
        ...
        flash_write_enable(id);
        flash_write(id, wr_cmd, 3);

        while (flash_is_busy(id));
}

__STATIC_INLINE void flash_mx_configure_dummy_cycles(HW_QSPIC_ID id, uint8_t dummy_cycles) __attribute__((always_inline));
__STATIC_INLINE void flash_mx_configure_dummy_cycles(HW_QSPIC_ID id, uint8_t dummy_cycles)
{
        uint8_t new_value;
        uint8_t dc_value;
...
__STATIC_INLINE void flash_mx_set_high_performance(HW_QSPIC_ID id)  __attribute__((always_inline));
__STATIC_INLINE void flash_mx_set_high_performance(HW_QSPIC_ID id)
{
        uint8_t cmd[] = { MX_READ_CONFIG_REGISTER };
        uint8_t conf_reg[2];
        uint8_t new_value;
...
__RETAINED_CODE static bool flash_mx_is_suspended(HW_QSPIC_ID id)
{
        ...
        status = flash_mx_read_security_register(id);
