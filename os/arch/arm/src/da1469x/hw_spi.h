 ...
 * \{
 * \brief Serial Pripheral Interface Controller
 */
...
typedef enum {
        ...
        HW_SPI_WORD_9BIT,       /**< 9 bits mode. Only valid in master mode */
} HW_SPI_WORD;
 ...
 * SPI_ON is cleared and SPI_RST is set.
 *
 * \param [in] id identifies SPI1 or SPI2
 *
 * \sa HW_SPI_ASSERT
...
#define HW_SPI_ASSERT(id)                                                                       \
        do {                                                                                    \
                ASSERT_WARNING(HW_SPI_REG_GETF(id, SPI_CTRL_REG, SPI_ON) == false);             \
                ASSERT_WARNING(HW_SPI_REG_GETF(id, SPI_CTRL_REG, SPI_RST) == true);             \
        } while (0);
 ...
 *
 */
__STATIC_INLINE void hw_spi_fifo_write32(HW_SPI_ID id, uint32_t val)
{
        SBA(id)->SPI_RX_TX_REG = val;
}
 ...
 *
 */
__STATIC_INLINE uint8_t hw_spi_get_interrupt_status(HW_SPI_ID id)
{
 ...
 *
 */
__STATIC_INLINE void hw_spi_clear_interrupt(HW_SPI_ID id)
{
...
__STATIC_INLINE void hw_spi_enable(HW_SPI_ID id, uint8_t on)
{
        ...
        if (on) {
                HW_SPI_REG_SETF(id, SPI_CTRL_REG, SPI_ON, true);
                HW_SPI_REG_SETF(id, SPI_CTRL_REG, SPI_RST, false);
        } else {
                HW_SPI_REG_SETF(id, SPI_CTRL_REG, SPI_ON, false);
                HW_SPI_REG_SETF(id, SPI_CTRL_REG, SPI_RST, true);
        }
}
 ...
 *
 */
__STATIC_INLINE void hw_spi_set_clock_phase(HW_SPI_ID id, HW_SPI_PHA phase)
{
 ...
 *
 */
__STATIC_INLINE void hw_spi_set_clock_polarity(HW_SPI_ID id, HW_SPI_POL pol)
{
        uint32_t on = hw_spi_is_enabled(id);
        hw_spi_enable(id, 0);                                   // close SPI block
        HW_SPI_ASSERT(id);
 ...
 *
 */
__STATIC_INLINE void hw_spi_set_do_level(HW_SPI_ID id, uint8_t spi_do)
{
        uint32_t on = hw_spi_is_enabled(id);
        hw_spi_enable(id, 0);                                   // close SPI block
        HW_SPI_ASSERT(id);
 ...
 *
 */
__STATIC_INLINE uint8_t hw_spi_get_do_level(HW_SPI_ID id)
{
 ...
 *
 */
__STATIC_INLINE void hw_spi_set_force_do(HW_SPI_ID id, uint8_t spi_force_do)
{
        uint32_t on = hw_spi_is_enabled(id);
        hw_spi_enable(id, 0);                                           // close SPI block
        HW_SPI_ASSERT(id);
 ...
 *
 */
__STATIC_INLINE uint8_t hw_spi_get_force_do(HW_SPI_ID id)
{
 ...
 *
 */
__STATIC_INLINE uint8_t hw_spi_is_tx_fifo_full(HW_SPI_ID id)
{
 ...
 *
 */
__STATIC_INLINE uint8_t hw_spi_get_di(HW_SPI_ID id)
{
 ...
 *
 */
__STATIC_INLINE void hw_spi_set_cs_ctrl(HW_SPI_ID id, uint8_t spi_en_ctrl)
{
        uint32_t on = hw_spi_is_enabled(id);
        hw_spi_enable(id, 0);                                           // close SPI block
        HW_SPI_ASSERT(id);
        HW_SPI_REG_SETF(id, SPI_CTRL_REG, SPI_EN_CTRL, spi_en_ctrl);    // set SPI_EN bit
        hw_spi_enable(id, on);                                          // open SPI block
}
 ...
 *
 */
__STATIC_INLINE uint8_t hw_spi_get_cs_ctrl(HW_SPI_ID id)
{
 ...
 *
 */
__STATIC_INLINE void hw_spi_set_9th_bit(HW_SPI_ID id, uint8_t val)
{
        uint32_t on = hw_spi_is_enabled(id);
        hw_spi_enable(id, 0);
        HW_SPI_ASSERT(id);
        // Get value of the SPI_EN_CTRL bit from the SPI control register
        HW_SPI_REG_SETF(id, SPI_CTRL_REG, SPI_9BIT_VAL, val);
        hw_spi_enable(id, on);
}
 ...
 *
 */
__STATIC_INLINE uint8_t hw_spi_get_9th_bit(HW_SPI_ID id)
{
        // Get value of the SPI_EN_CTRL bit from the SPI control register
        return HW_SPI_REG_GETF(id, SPI_CTRL_REG, SPI_9BIT_VAL);
}

/**
 * \brief Initialize peripheral divider register - select and enable SPI clock
 *
...
/**
 * \brief Check if SPI CS is high
 *
 * \param [in] id identifies SPI1 or SPI2
 *
 * \return true if CS is high
 *
 */
 ...
 *
 */
__STATIC_INLINE void hw_spi_set_dma_priority(HW_SPI_ID id, uint8_t priority)
{
        uint32_t on = hw_spi_is_enabled(id);
        hw_spi_enable(id, 0);
        HW_SPI_ASSERT(id);
        HW_SPI_REG_SETF(id, SPI_CTRL_REG, SPI_PRIORITY, priority);
        hw_spi_enable(id, on);
}
 ...
 *
 */
__STATIC_INLINE uint8_t hw_spi_get_dma_priority(HW_SPI_ID id)
{
        return HW_SPI_REG_GETF(id, SPI_CTRL_REG, SPI_PRIORITY);
}
...
__STATIC_INLINE uint8_t hw_spi_is_busy(HW_SPI_ID id)
{
        ...
        return HW_SPI_REG_GETF(id, SPI_CTRL_REG, SPI_BUSY);
}
...
/**
 * \brief Set DMA TX request to be generated when tx buffer is free
 *
 * \param [in] id identifies SPI1 or SPI2
 *
 *
 */
__STATIC_INLINE void hw_spi_set_quick_dma_txreq(HW_SPI_ID id)
{
        uint32_t on = hw_spi_is_enabled(id);
        hw_spi_enable(id, 0);
        HW_SPI_ASSERT(id);
