...
{
        if (idx <= HW_OTP_MAX_IDX)
        {
                return hw_otpc_word_read((MEMORY_OTP_USER_DATA_KEYS_INDEX_START / 4) + idx) ? true :
                                                                                              false;
        } else {
                return false;
        }
...
{
        if (!hw_aes_hash_valid_user_data_keys_idx(idx))
        {
                return 0;
        }

        return (MEMORY_OTP_BASE_P + MEMORY_OTP_USER_DATA_KEYS_PAYLOAD_START +
                                                                (idx * HW_OTP_USER_DATA_KEY_SIZE));
}
#endif /* dg_configAES_USE_SECURE_DMA_CHANNEL */
...
void hw_aes_hash_store_keys(hw_aes_key_size key_size, const uint8 *aes_keys, hw_aes_hash_key_exp_t key_exp)
{
        ...
        if (((uint32_t) aes_keys >= MEMORY_OTP_BASE_P + MEMORY_OTP_USER_DATA_KEYS_PAYLOAD_START) &&
                        ((uint32_t) aes_keys < MEMORY_OTP_BASE_P + MEMORY_OTP_USER_DATA_KEYS_PAYLOAD_END)) {
