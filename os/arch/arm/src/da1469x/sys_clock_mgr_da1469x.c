...
static bool rcx_calibration_is_on = false;
...
static OS_BASE_TYPE xtal32m_is_ready(BaseType_t *xHigherPriorityTaskWoken)
{
        ...
        if (xtal32m_settled_notification == false) {
                ...
                if ((dg_configLP_CLK_SOURCE == LP_CLK_IS_ANALOG) && (dg_configUSE_LP_CLK == LP_CLK_RCX)) {
                        if ((sysclk > sysclk_RC32) && (sysclk < sysclk_LP)) {
                                // Start calibration
...
void cm_clk_init_low_level_internal(void)
{
        ...
        if (dg_configLP_CLK_SOURCE == LP_CLK_IS_DIGITAL) {
                ...
                hw_clk_configure_lpclk(LP_CLK_IS_RCX);          // Configure RCX
...
void cm_rcx_calibrate(void)
{
        ...
        hw_clk_start_calibration(CALIBRATE_RCX, CALIBRATE_REF_DIVN, RCX_CALIBRATION_CYCLES_WUP);
...
void cm_sys_clk_init(sys_clk_t type)
{
        ...
        if ((dg_configLP_CLK_SOURCE == LP_CLK_IS_ANALOG) && (dg_configUSE_LP_CLK == LP_CLK_RCX)) {
                NVIC_SetPendingIRQ(XTAL32M_RDY_IRQn);       // XTAL32M has already been settled
        }
...
cm_sys_clk_set_status_t cm_sys_clk_set(sys_clk_t type)
{
		...
		if (type == sysclk_XTAL32M)
			pm_update_clk_time(clk_sysclk_default);
		else if (type == sysclk_PLL96)
			pm_update_clk_time(clk_sysclk_max);
...
static bool sys_clk_check_div1(void)
{
        ...
        if (hw_pd_check_com_status()) {
                ...
                // Check SDADC
                if (REG_GETF(SDADC, SDADC_CTRL_REG, SDADC_EN) && (tmp & REG_MSK(CRG_COM, CLK_COM_REG, SDADC_CLK_SEL))) {
                        return true;
                }
...
{
        if (!rcx_calibration_is_on) {
                hw_clk_start_calibration(CALIBRATE_RCX, CALIBRATE_REF_DIVN, RCX_CALIBRATION_CYCLES_WUP);
                rcx_calibration_is_on = true;
        }
}
