...
//#               include "qspi_w25q128jw_pig.h"
#               include "qspi_w25q256jw_pm.h"
#               include "qspi_gd25lq256.h"
...
//                &flash_w25q128jwpig_config,
                &flash_gd25lq256_config,
                &flash_w25q256jwpm_config,
                &flash_w25q80ew_config,
 ...
 */
__STATIC_INLINE HW_QSPIC_ID flash_get_addr_id(uint32_t addr, uint32_t size) __attribute__((always_inline));

__STATIC_INLINE HW_QSPIC_ID flash_get_addr_id(uint32_t addr, uint32_t size)
{
...
__STATIC_INLINE uint32_t flash_get_zero_based_addr(uint32_t addr) __attribute__((always_inline));

__STATIC_INLINE uint32_t flash_get_zero_based_addr(uint32_t addr)
{
#if dg_configUSE_HW_QSPI2
        if (addr >= dg_configQSPI2_FLASH_BASE_ADDR) {
...
__RETAINED_CODE bool qspi_process_operations(void)
{
        ...
        if (qspi_active_op != NULL) {
                ...
                if (qspi_active_op->suspended) {
                        ...
                        qspi_resume(id);
                        hw_clk_delay_usec(FLASH_SUS_DELAY); // Guard time until SUSPEND can be issued!
 ...
 */
__STATIC_INLINE void flash_set_bus_mode(HW_QSPIC_ID id, HW_QSPI_BUS_MODE mode) __attribute__((always_inline));

__STATIC_INLINE void flash_set_bus_mode(HW_QSPIC_ID id, HW_QSPI_BUS_MODE mode)
{
 ...
 */
__STATIC_INLINE void flash_set_automode(HW_QSPIC_ID id, bool automode) __attribute__((always_inline));

__STATIC_INLINE void flash_set_automode(HW_QSPIC_ID id, bool automode)
{
...
__STATIC_INLINE void fast_write_to_fifo32(uint32_t start, uint32_t end, uint32_t dest)
{
        asm volatile(   "copy:                                  \n"
                        "       ldmia %[start]!, {r3}           \n"
                        "       str r3, [%[dest]]               \n"
...
__RETAINED_CODE void qspi_int_configure(HW_QSPIC_ID id)
{
        ...
        qspi_init_config.read_instruction.inst = QSPI_GET_DEVICE_PARAM(idx, read_instr);
        ...
        qspi_init_config.read_instruction.addr_phase = QSPI_GET_DEVICE_PARAM(idx, read_addr_phaze);
...
//         qspi_int_activate_command_entry_mode(id);
        ...
        hw_qspi_set_instructions(id, &qspi_init_config);
         ...
         hw_qspi_set_extra_byte(id, QSPI_GET_DEVICE_PARAM(idx, extra_byte), QSPI_GET_DEVICE_PARAM(idx, send_extra_byte), HW_QSPI_BUS_MODE_QUAD, 0);
