 ...
 *\param [in] N_CMDs     Defines the number of commands (including the current) to be pushed to PGs on
 *                       a single trigger
...
__STATIC_INLINE uint16_t hw_smotor_get_fifo_command(uint8_t index)
{
        ...
        return *(uint16_t*)(SMOTOR->CMD_TABLE_BASE + index * 2);
}
 ...
 *
 * \return true when wave is added, false when Wave Table is full
 */
