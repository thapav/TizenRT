                                           ...
                                           HW_CACHE_RAMSZ ramsz)
{
        ...
        REG_SET_FIELD(CACHE, CACHE_CTRL3_REG, CACHE_LINE_SIZE_RESET_VALUE, val, linesz);
        REG_SET_FIELD(CACHE, CACHE_CTRL3_REG, CACHE_ASSOCIATIVITY_RESET_VALUE, val, assoc);
        REG_SET_FIELD(CACHE, CACHE_CTRL3_REG, CACHE_RAM_SIZE_RESET_VALUE, val, ramsz);
        CACHE->CACHE_CTRL3_REG = val;

        /* reset the cache controller to apply new configuration */
        REG_SETF(CACHE, CACHE_CTRL3_REG, CACHE_CONTROLLER_RESET, 1);
