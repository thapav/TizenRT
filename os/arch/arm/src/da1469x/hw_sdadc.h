 ...
 */
typedef enum {
        HW_SDADC_CLOCK_DIVN = 0,       /**< divn_clk always 32MHz (default) */
        HW_SDADC_CLOCK_SYS_CLK = 1     /**< System clock (sys_clk) can be any oscillator,
                                            the PLL or even an externally supplied digital clock */
} HW_SDADC_CLOCK;
 ...
 */
typedef struct {
        HW_SDADC_CLOCK         clock;             /**< Clock source */
        ...
        uint16_t               vref_voltage;      /**< Reference voltage (mV) - MUST be set to 1.2V when vref source is internal */
...
{
        REG_SETF(CRG_COM, CLK_COM_REG, SDADC_CLK_SEL, clock);
}
...
{
        return (HW_SDADC_CLOCK) REG_GETF(CRG_COM, CLK_COM_REG, SDADC_CLK_SEL);
}
 ...
 *
 */
__STATIC_INLINE int32_t hw_sdadc_result_reg_to_millivolt(sdadc_config *cfg)
{
        int32_t converted;
        uint16_t max;
 ...
 *
 */
__STATIC_INLINE int32_t hw_sdadc_get_voltage(sdadc_config *cfg)
{
        hw_sdadc_start();     // Start AD conversion
